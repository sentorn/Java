...
public class ChangePanel extends JPanel {
	...
	public ChangePanel(Solutions s, EditPanel e) {
		this.e = e;
		this.s = s;
		this.setLayout(null);
	...
	boolean checkField() {
		...
		if (!tf.getText().equals("") && s.employeeExist(tf.getText())) {
			int n = s.getEmployeeIndex(tf.getText());
			e.getNameF().setText(s.getEmpList().get(n).getName());
			e.getLastName().setText(s.getEmpList().get(n).getLastName());
			e.getAge().setText(String.valueOf(s.getEmpList().get(n).getAge()));
			e.getSalary().setText(String.valueOf((int)s.getEmpList().get(n).getSalary()));
			e.getAddress().setText(s.getEmpList().get(n).getAddress());
			res = true;
			clearPanel();
		}
		return res;
